<template>
  <UAccordion :key="route.path" :items="items as AccordionItem[]" :multiple="multiple" :ui="ui" v-bind="attrs">
    <template #default="{ item, open }">
      <ULink :class="[ui.button.base, level > 0 && ui.button.level]" :active-class="ui.button.active" :inactive-class="ui.button.inactive">
        ...
        <UIcon v-if="!item.disabled" :name="ui.button.trailingIcon.name" :class="[ui.button.trailingIcon.base, open ? ui.button.trailingIcon.active : ui.button.trailingIcon.inactive]" />
      </ULink>
    </template>

    <template v-for="({ label }, index) in links" :key="index" #[label.toLowerCase()]="{ item }">
      <UNavigationTree :links="item.children" :level="level + 1" :default-open="defaultOpen" :multiple="multiple" :class="ui.tree" />
    </template>
  </UAccordion>
</template>
...
const config = computed(() => {
  const wrapper: string = twJoin("space-y-3", props.level > 0 && "border-l border-gray-200 dark:border-gray-800 -ml-px hover:border-gray-300 dark:hover:border-gray-700");
