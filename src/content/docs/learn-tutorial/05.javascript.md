...
![JavaScript面试题.png](/images/docs/learntutorial/05.png)

## 一、数据类型

### 1. JavaScript有哪些数据类型，它们的区别？ `1`
...
![image](/images/docs/learntutorial/05-0.png)
...
![image](/images/docs/learntutorial/05-1.png)
...
![img](/images/docs/learntutorial/05-2.png)

![img](/images/docs/learntutorial/05-3.png)
...
![image](/images/docs/learntutorial/05-4.png)
...
![image](/images/docs/learntutorial/05-5.png)
...
![image](/images/docs/learntutorial/05-6.png)
...
### 10. Unicode、UTF-8、UTF-16、UTF-32的区别？---了解即可 `3`
...
### 12. 为什么函数的 arguments 参数是类数组而不是数组？如何遍历类数组? `2`
...
### 15. escape、encodeURI、encodeURIComponent 的区别 `3`

- encodeURI 是对整个 URI 进行转义，将 URI 中的非法字符转换为合法字符，所以对于一些在 URI 中有特殊意义的字符不会进行转义。
- encodeURIComponent 是对 URI 的组成部分进行转义，所以一些特殊字符也会得到转义。
- escape 和 encodeURI 的作用相同，不过它们对于 unicode 编码为 0xff 之外字符的时候会有区别，escape 是直接在字符的 unicode 编码前加上 %u，而 encodeURI 首先会将字符转换为 UTF-8 的格式，再在每个字节前加上 %。

### 16. 对AJAX的理解，实现一个AJAX请求 `1`
...
### 17. JavaScript为什么要进行变量提升，它导致了什么问题？ `1`
...
### 18. 什么是尾调用，使用尾调用有什么好处？ `3`

尾调用指的是函数的最后一步调用另一个函数。代码执行是基于执行栈的，所以当在一个函数里调用另一个函数时，会保留当前的执行上下文，然后再新建另外一个执行上下文加入栈中。使用尾调用的话，因为已经是函数的最后一步，所以这时可以不必再保留当前的执行上下文，从而节省了内存，这就是尾调用优化。但是 ES6 的尾调用优化只在严格模式下开启，正常模式是无效的。

### 19.  ES6模块与CommonJS模块有什么异同？  `2`
...
![image](/images/docs/learntutorial/05-7.png)
...
![image](/images/docs/learntutorial/05-8.jpg)
...
### 3. 实现call、apply 及 bind 函数（建议看一下[鲨鱼哥的掘金手写](https://juejin.cn/post/6968713283884974088#heading-9)） `1`
...
![img](/images/docs/learntutorial/05-9.png)
...
### 6. Promise.all和Promise.race的区别的使用场景 `2`
...
![img](/images/docs/learntutorial/05-10.png)
...
### 13. 什么是回调函数？回调函数有什么缺点？如何解决回调地狱问题？ `3`
...
### 14. setTimeout、setInterval、requestAnimationFrame 各有什么特点？ `3`
